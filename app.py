...
def search_movies(user_id):
    ...
    try:
        ...
        query_db_result = list(query_db_result)
        def num_reviews_key(movie):
            num_reviews = (movie["tomatoes"]["viewer"]["numReviews"], None) if "tomatoes" in movie and "viewer" in movie["tomatoes"] else (None, None)
            num_reviews_value = num_reviews[0]
            num_reviews_int = int(num_reviews_value) if num_reviews_value is not None else 0
            return num_reviews_int
        query_db_result.sort(key=num_reviews_key)
        movies_data = []
        for movie in query_db_result[:30]:
            print(movie['title'])
